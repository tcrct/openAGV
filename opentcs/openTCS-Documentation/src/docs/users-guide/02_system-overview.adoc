...
A location type has only one relevant attributes:
...
* An optional set of _dependencies_, i.e. references to other transport orders that need to be processed before the transport order.
  ...
  This can optionally be achieved by also setting the _intended vehicle_ attributes of the transport orders.)
